...
def create_user():
    ...
    # Handle POST request - support both form styles
    full_name = request.form.get('full_name')
    email_prefix = request.form.get('email_prefix')
    
    if full_name and email_prefix:
        # New role-based form with @caios.app email
        username = email_prefix
        password = 'temp123'  # Temporary password - user will reset
        email = f"{email_prefix}@caios.app"
    ...
    result = user_manager.create_user(username, password, email, role)
    
    if result['success']:
        if full_name and email_prefix:
            # New form - redirect to success page
            flash(f'User {email} created successfully!', 'success')
            return redirect(url_for('user_created', email=email))
        else:
            # Legacy form - back to admin
...
def user_created():
    ...
    return render_template('user_created.html', email=email)

@app.route('/user-directory')
@login_required
@role_required('user_admin')
def user_directory():
    """User directory dashboard showing all created users"""
    # Load users from the new user creation system
    ...
    # Also load existing users from the main users.json for compatibility
    existing_users = []
    try:
    ...
    return render_template('user_directory.html', users=all_users)
...
def toggle_user(username):
    ...
    if user_data:
        new_status = not user_data['active']
        result = user_manager.update_user_status(username, new_status)
...
@app.route('/')
def index():
    """Main upload page - redirect to login if not authenticated."""
    if 'user' not in session:
        return redirect(url_for('login'))
    else:
        return redirect(url_for('dashboard'))
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            if result['success']:
                ...
                flash(f'Successfully processed {coils_count} coils from {filename} and added to spreadsheet!')
                return redirect(url_for('dashboard'))
            else:
                flash(f'Error processing BOL: {result["error"]}', 'error')
...
def add_supplier():
    ...
    try:
        ...
        supplier_key = name.lower().replace(' ', '_')
        if supplier_key in prompts:
            return jsonify({
                'success': False,
                'message': 'Supplier already exists'
            }), 400
        
        # Add new supplier with default prompt
        prompts[supplier_key] = {
            'name': name,
        ...
        with open('supplier_prompts.json', 'w') as f:
